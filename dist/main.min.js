...
class HangmanGame {
    ...
    handleKeyboardInput = (event) => {
        if (this.state.status !== "playing" || event.repeat)
            return;
        const keyLetter = this.normalizeLetter(event.key);
        if (!this.isPlayableLetter(keyLetter))
            return;
    ...
    bindEvents() {
        ...
        this.refs.lettersBoard.addEventListener("click", (event) => {
            const target = event.target;
            const letterButton = target.closest("button[data-letter]");
    ...
    hasSolvedWord() {
        const requiredLetters = new Set(this.getPlayableLetters(this.state.word));
        return [...requiredLetters].every((letter) => this.state.guessed.has(letter));
    }
    getPlayableLetters(value) {
    ...
    createState() {
        ...
        if (!entriesInCategory || entriesInCategory.length === 0) {
            throw new Error(`No words available in the selected category: ${category}`);
        }
    ...
    collectDomRefs() {
        const gallowsParts = Array.from(document.querySelectorAll("[data-part-index]"));
        if (gallowsParts.length === 0) {
            throw new Error("Missing required hangman parts.");
        }
        return {
            category: this.must("[data-category]"),
            hintType: this.must("[data-hint-type]"),
            hintText: this.must("[data-hint-text]"),
            hintMeta: this.must("[data-hint-meta]"),
            wordBoard: this.must("[data-word-board]"),
            lettersBoard: this.must("[data-letters-board]"),
            missCount: this.must("[data-miss-count]"),
            maxMissCount: this.must("[data-max-miss]"),
            gallowsParts,
            resultOverlay: this.must("[data-result-overlay]"),
            resultTitle: this.must("[data-result-title]"),
            resultMessage: this.must("[data-result-message]"),
            newGameButton: this.must("[data-action='new-game']"),
            playAgainButton: this.must("[data-action='play-again']"),
        };
    }
}
